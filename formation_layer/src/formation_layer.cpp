...
namespace formation_layer_namespace
{
    ...
    void FormationLayer::onInitialize()
    {
        ...
        if(this->service_flag){
            this->mecService = this->nh_.advertiseService("mec_Info", &FormationLayer::mecInfoServiceCallback, this);
        ...
        if(this->nh_.searchParam("robots_number", robots_number_key)){
            ...
            for (int i = 0; i < robots_number; i++){
                ...
                posesCallbacks.push_back ([this, i, robot_id](const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg){
                    // this->robot_poses.push_back(*msg.get());
                    this->robot_positions[robot_id] = *msg;
                }); 

                //create and save subscribers 
                posesSubscribers.push_back(this->nh_.subscribe<geometry_msgs::PoseWithCovarianceStamped>(topic_name, 10, posesCallbacks[i]));
                        ...
                        double* max_x, double* max_y)
    {
        ...
        if(!robot_positions.empty()){ 
            ...
            //calculate the formation footprint from the footprints points
            ...
            this->bounding_box = calculateBoundingBox(formation_footprint);
